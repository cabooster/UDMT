...
# Compatible with different versions of PyTorch
try:
...
def resnet18(pretrained=True, extra_dim=0):
    ...
    if pretrained:
        load_weights_add_extra_dim(model, load_state_dict_from_url(model_urls['resnet18']), extra_dim)
    return model

def resnet50(pretrained=True, extra_dim=0):
    model = ResNet(Bottleneck, [3, 4, 6, 3], extra_dim)
    if pretrained:
        load_weights_add_extra_dim(model, load_state_dict_from_url(model_urls['resnet50']), extra_dim)
    return model
