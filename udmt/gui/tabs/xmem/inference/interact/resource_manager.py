...
class ResourceManager:
    def __init__(self, config, divide_num):
        # determine inputs
        ...
        self.resize_scale = self.size/min(image.shape[0], image.shape[1])
    ...
    def save_mask(self, ti, mask):
        ...
        # Special handling for first frame (ti=0) to merge with auto prediction
        if ti == 0 and path.exists(mask_path):
            # Load existing auto prediction mask
            try:
                existing_mask = np.array(Image.open(mask_path))
                # Check if existing mask has content (auto prediction)
                if np.any(existing_mask > 0):
                    print(f"🔄 Merging manual click with existing auto prediction mask...")
                    # Merge: combine auto prediction (existing) with manual mask (new)
                    # Manual mask takes precedence but preserve existing predictions
                    merged_mask = np.maximum(existing_mask, mask)
                    mask = merged_mask
                    print(f"✅ Successfully merged masks for first frame")
            except Exception as e:
                print(f"⚠️ Warning: Could not merge with existing mask: {e}")
                # Continue with original mask if merge fails
