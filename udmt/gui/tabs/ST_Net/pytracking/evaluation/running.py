...
from ..evaluation import Sequence
from ..evaluation.tracker_multi import Tracker
from ...ltr.data.image_loader import imwrite_indexed


def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict, object_id = None):
    ...
    for key, data in output.items():
        ...
        if key == 'target_bbox':
            ...
            #         save_bb(bbox_file, d)
                ...
                bbox_file = '{}_{}.txt'.format(base_results_path, object_id)
                save_bb(bbox_file, data)
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, visdom_info=None, search_scale = 2.0, target_sz_bias = 0, object_id=None,gui_param = None ):
    ...
    def _results_exist():
        if object_id is None:
            ...
            bbox_file = '{}/{}_{}.txt'.format(tracker.results_dir, seq.name, object_id)
            if os.path.isfile(bbox_file):
                os.remove(bbox_file)
    ...
    if debug:
        output, early_stop_flag = tracker.run_sequence(seq, debug=debug, visdom_info=visdom_info)
    else:
        # try:
    ...
    # if not debug:
    #     _save_tracker_output(seq, tracker, output, object_id)

    return early_stop_flag


def run_dataset(seq, trackers, debug=False, threads=0, visdom_info=None, search_scale = 2.0, target_sz_bias=0, obj_id=None,gui_param =None):
