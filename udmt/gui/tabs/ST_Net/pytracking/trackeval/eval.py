...
class Evaluator:
    ...
    @_timing.time
    def evaluate(self, dataset_list, metrics_list,start_frame,end_frame, metric_type='HOTA', show_progressbar=False):
                        ...
                        if show_progressbar and TQDM_IMPORTED:
                            ...
                            for curr_seq in tqdm.tqdm(seq_list_sorted):
                                res[curr_seq] = eval_sequence(curr_seq, dataset, tracker, class_list, metrics_list,
                                                              metric_names,start_frame,end_frame)
                        else:
                        ...
                        if config['OUTPUT_EMPTY_CLASSES'] or num_dets > 0:
                            for metric, metric_name in zip(metrics_list, metric_names):
                                ...
                                if config['PRINT_RESULTS'] and config['PRINT_ONLY_COMBINED']:
                                    ...
                                    if metric_name == metric_type: # HOTA
                                    # if tracker == 'ours':
                                        metric_ = metric.print_table(table_res, tracker_display_name, c_cls,return_flag=True)
                                        metric_num.append(metric_)
                                    else:
                                        if ((metric_name == 'CLEAR') & (metric_type == 'IDSW')):
                                            metric_ = metric.print_table(table_res, tracker_display_name, c_cls,return_flag=True)
                                            metric_num.append(table_res['COMBINED_SEQ']['IDSW'])
                                        else:
        ...
        metric_num = metric_num[:4]
        print('DLC:',metric_num[0],'idtrackerai:',metric_num[1],'ours:',metric_num[2],'sleap:',metric_num[3])
        return output_res, output_msg, metric_num


@_timing.time
def eval_sequence(seq, dataset, tracker, class_list, metrics_list, metric_names,start_frame,end_frame):
    """Function for evaluating a single sequence"""

    raw_data = dataset.get_raw_seq_data(tracker, seq,start_frame,end_frame)
    seq_res = {}
