...
from udmt.gui.tabs.ST_Net.pytracking.tracker.base import BaseTracker
from udmt.gui.tabs.ST_Net.pytracking.libs import dcf
from udmt.gui.tabs.ST_Net.pytracking.features import augmentation
from udmt.gui.tabs.ST_Net.pytracking.features.preprocessing import numpy_to_torch,sample_patch_multiscale, sample_patch_transformed
from udmt.gui.tabs.ST_Net.pytracking.libs.tensorlist import TensorList
from udmt.gui.tabs.ST_Net.pytracking.utils.plotting import show_tensor, plot_graph
from udmt.gui.tabs.ST_Net.ltr.models.target_classifier.initializer import FilterInitializerZero
from udmt.gui.tabs.ST_Net.ltr.models.layers import activation
from udmt.gui.tabs.ST_Net.ltr.data import bounding_box_utils as bbutils
...
class TrDiMP(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        # Setup scale bounds
        self.min_scale_factor = torch.max(10 / self.base_target_sz)  # !!! torch.max(10 / self.base_target_sz)
        self.max_scale_factor = torch.min(self.image_sz / self.base_target_sz) # !!! torch.min(self.image_sz / self.base_target_sz)
    ...
    def extract_patch(self,image):
        ...
        im = numpy_to_torch(image)
        im_patches, patch_coords = sample_patch_multiscale(im, self.get_centered_sample_pos(),
                                                        self.target_scale * self.params.scale_factors,
                                                        self.img_sample_sz,mode=self.params.get('border_mode', 'replicate'),
                                                        max_scale_change=self.params.get('patch_max_scale_change', None))
        return im_patches, patch_coords
    def extract_backbone_parallel(self,im_patches_multibatch,ani_num):
        im_patches_multibatch_tensor = torch.cat(im_patches_multibatch, dim=0)
        # im_patches_multibatch_tensor = torch.stack(im_patches_multibatch, dim=0)
        # with autocast():
        # self.net.feature_extractor.half()
        # im_patches_multibatch_tensor = im_patches_multibatch_tensor.half()
        with torch.no_grad():
            backbone_feat_multibatch = self.net.extract_backbone(im_patches_multibatch_tensor)
        # self.net.float()
        # backbone_feat_multibatch.float()
        backbone_feat_list = []
        for i in range(ani_num):
            ordered_dict = OrderedDict([('layer2', backbone_feat_multibatch['layer2'][i].unsqueeze(0)), ('layer3', backbone_feat_multibatch['layer3'][i].unsqueeze(0))])
            backbone_feat_list.append(ordered_dict)
        return backbone_feat_list
    def track_speed_up(self, backbone_feat, sample_coords, im_patches, ani_id = 0):
        frame_id = self.frame_num
        ...
        if debug_save:
            ...
            if frame_id > debug_frame_id: # 2294
                if ani_id == debug_ani_id:
                    debug_save_path = os.path.abspath(os.path.join(os.getcwd(), '..', 'ltr' ,'save_tmp_trdimp_debug'))
                    # debug_save_path = 'E:/01-LYX/new-research/TransformerTrack-main/TransformerTrack-main/ltr/save_tmp_trdimp_debug'
                    if not os.path.exists(debug_save_path):
                        os.makedirs(debug_save_path)
                    save_patch = im_patches[0].cpu().numpy()
                    save_patch = np.transpose(save_patch, (1, 2, 0))
                    cv2.imwrite(debug_save_path + '/patch_inference_' + str(frame_id) +'.jpg', save_patch)


        # Extract classification features
        # with autocast():
        t2 = time.time()
        x_clf = self.get_classification_features(backbone_feat) # x_clf: 1,512,22,22
        t3 = time.time()
        decoded_x, test_x = self.transformer_decoder(x_clf) #decoded_x: 484,1,512 test_x: 1,512,22,22
        t4 = time.time()
        ...
        scores_raw = self.classify_target(test_x) # self.target_filter test_x

        # print('sample_coords:',sample_coords)
        # print('sample_pos:', sample_pos)
        # print('sample_scales:',sample_scales)
        if debug_save:
            # print('frame_id : ',frame_id)
            if frame_id > debug_frame_id: # 2294
                if ani_id == debug_ani_id:
                    debug_scores_save_path = os.path.abspath(os.path.join(os.getcwd(), '..', 'ltr' ,'save_score_debug'))
                    # debug_scores_save_path = 'E:/01-LYX/new-research/TransformerTrack-main/TransformerTrack-main/ltr/save_score_debug'
                    if not os.path.exists(debug_scores_save_path):
                        os.makedirs(debug_scores_save_path)
                    save_patch = scores_raw[0].cpu().numpy()
                    # save_patch = np.transpose(save_patch, (1, 2, 0))
                    io.imsave(debug_scores_save_path + '/patch_inference_score_' + str(frame_id) +'.tif', save_patch, check_contrast=False)
                    # cv2.imwrite(debug_scores_save_path + '/patch_inference_score_' + str(frame_id) +'.jpg', save_patch)
        t5 = time.time()
        # Localize the target
        ...
        return out, score_map, s, flag, sample_scales, sample_pos, test_x, x_clf, scale_ind


    def track(self, image, ani_id = 0):
        self.debug_info = {}
        frame_id = self.frame_num
        self.frame_num += 1
        self.debug_info['frame_num'] = self.frame_num
        ...
        backbone_feat, sample_coords, im_patches = self.extract_backbone_features(im, self.get_centered_sample_pos(),
                                                                      self.target_scale * self.params.scale_factors,
                                                                      self.img_sample_sz)
        # extract_backbone_features: self.feature_extractor(im, layers) of layer3
        ...
        x_clf = self.get_classification_features(backbone_feat) # x_clf: 1,512,22,22
        t3 = time.time()
        decoded_x, test_x = self.transformer_decoder(x_clf) #test_x: 1,512,22,22
        t4 = time.time()
        ...
        if update_flag and self.params.get('update_classifier', False):
            ...
            if (self.frame_num - 1) % self.params.transformer_skipping == 0:
                ...
                # transformer_memory = []
                # for i in range(100):
                #     transformer_memory.append(self.x_clf)
                ######
    ...
    def update_post(self, flag, refine_pos, sample_scales, sample_pos, test_x, x_clf, s, scale_ind, ani_id = 0, frame_id = 0):
        # Update position and scale
        if flag != 'not_found':
            ## change

            if self.params.get('use_classifier', True):
                self.update_state(refine_pos, sample_scales[scale_ind])

        # print('tracker.pos in update', self.pos)
        # ------- UPDATE ------- #
        update_flag = flag not in ['not_found', 'uncertain']
        hard_negative = (flag == 'hard_negative')
        learning_rate = self.params.get('hard_negative_learning_rate', None) if hard_negative else None
    ...
    def track_combine(self, image, ani_id=0, frame_id=0):
        self.debug_info = {}
        ...
        if debug_save:
            ...
            if frame_id > debug_frame_id:  # 2294
                if ani_id == debug_ani_id:
                    ...
                    save_patch = scores_raw[0].cpu().numpy()
                    # save_patch = np.transpose(save_patch, (1, 2, 0))
                    io.imsave(debug_scores_save_path + '/patch_inference_score_' + str(frame_id) + '.tif', save_patch,
        ...
        if update_flag and self.params.get('update_classifier', False):
            ...
            if (self.frame_num - 1) % self.params.transformer_skipping == 0:
                ...
                if self.x_clf.shape[0] < self.params.transformer_memory_size:
                    ...
                    self.x_clf = torch.cat([x_clf, self.x_clf[:-1, ...]], dim=0)
                with torch.no_grad():  #### debug
                    self.transformer_memory, _ = self.net.classifier.transformer.encoder(self.x_clf.unsqueeze(1),
                                                                                         pos=None)
                ######
                # transformer_memory = []
                # for i in range(100):
    ...
    def localize_advanced(self, scores, sample_pos, sample_scales):
        ...
        output_sz = score_sz - (self.kernel_size + 1) % 2 #kernal [4,4]
        score_center = (score_sz - 1)/2
    ...
    def extract_backbone_features(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        debug = False
        if debug:
            debug_save_path = os.path.abspath(os.path.join(os.getcwd(), '..', 'ltr' ,'save_tmp_trdimp'))
            # debug_save_path = 'E:/01-LYX/new-research/TransformerTrack-main/TransformerTrack-main/ltr/save_tmp_trdimp'
            if not os.path.exists(debug_save_path):
                os.makedirs(debug_save_path)
            save_patch = im_patches[0].cpu().numpy()
            save_patch = np.transpose(save_patch, (1, 2, 0))
            cv2.imwrite(debug_save_path+'/patch_inference.jpg', save_patch)
        # copied_tensors = [im_patches.clone() for _ in range(10)]
        # result_tensor = torch.cat([im_patches] + copied_tensors, dim=0)
        # start_time1 = time.time()
        with torch.no_grad():
            backbone_feat = self.net.extract_backbone(im_patches)
            # record_time1 = time.time()
            # backbone_feat1 = self.net.extract_backbone(result_tensor)
            # record_time2 = time.time()
        # print('extract_backbone_features')
        # print(record_time1-start_time1)
        # print(record_time2-record_time1)
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        x = self.get_classification_features(init_backbone_feat) # x 13,512,22,22
        ...
        with torch.no_grad(): ###debug
            for i in range(x.shape[0]):
                _, cur_encoded_feat = self.net.classifier.transformer.decoder(x[i,...].unsqueeze(0).unsqueeze(0), memory=self.transformer_memory, pos=self.transformer_label, query_pos=None) # memory爆炸
                if i == 0:
    ...
    def refine_target_box(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        ##########
        # if self.frame_num > 750:
        #     print('!')
        new_poss_list = []
        for predictt in output_boxes:
            new_poss = predictt[:2] + predictt[2:] / 2
            new_poss = (new_poss.flip((0,)) - (self.img_sample_sz - 1) / 2) * sample_scale + sample_pos
        ...
        #############
        # distance_iou = self.pos_iounet - self.pos
        # distance_iou = distance_iou.numpy()
        # distance_cross_combine = np.sqrt(np.sum(distance_iou ** 2))
        # print(self.frame_num,distance_cross_combine)
        #############
