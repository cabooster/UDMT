...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 num_test_frames, num_train_frames=1, processing=no_processing, frame_sample_mode='causal',train_num_per_sequence = None):
        """
        args:
        ...
        self.train_num_per_sequence = train_num_per_sequence
    ...
    def _sample_visible_ids(self, visible, num_ids=1, min_id=None, max_id=None):
        ...
        if len(valid_ids) == 0:
            return None
        try:
            result = random.sample(valid_ids, k=num_ids)
        except:
            result = random.choices(valid_ids, k=num_ids) #!!
        return result
    ...
    def __getitem__(self, index):
        ...
        while not enough_visible_frames:
            ...
            # Sample frames
            seq_info_dict = dataset.get_sequence_info(seq_id,self.train_num_per_sequence)
        ...
        if is_video_dataset:
            ...
            if self.frame_sample_mode == 'interval':
                ...
                while test_frame_ids is None:
                    ...
                    prev_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_train_frames - 1,
                                                              min_id=base_frame_id[0] - train_gap - gap_increase,
                                                              max_id=base_frame_id[0]) #!!!
                    ...
                    test_frame_ids = self._sample_visible_ids(visible, min_id=train_frame_ids[0] + self.max_gap + gap_increase - train_gap,
                                                              max_id=train_frame_ids[0] + self.max_gap + gap_increase,
                                                              num_ids=self.num_test_frames) #!!!
        ...
        train_frames, train_anno = dataset.get_frames(seq_id, train_frame_ids, self.train_num_per_sequence, seq_info_dict)
        test_frames, test_anno = dataset.get_frames(seq_id, test_frame_ids, self.train_num_per_sequence, seq_info_dict)
...
class DiMPSampler(TrackingSampler):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,train_num_per_sequence,
        ...
        super().__init__(datasets=datasets, p_datasets=p_datasets, samples_per_epoch=samples_per_epoch, max_gap=max_gap,
                         ...
                         frame_sample_mode=frame_sample_mode,train_num_per_sequence = train_num_per_sequence)
