...
from udmt.gui.tabs.ST_Net.ltr.data.image_loader import jpeg4py_loader
...
class Got10k(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        seq_ids = [x for x in seq_ids if x <= 500]
    ...
    def _read_bb_anno(self, seq_path, train_num_per_sequence, seq_id=0):
        # bb_anno_file = os.path.join(seq_path, "groundtruth.txt")
        bb_anno_file =  self.root + '/label/' + self.sequence_list[seq_id] + '.txt'
        gt = pandas.read_csv(bb_anno_file, delimiter=',', header=None, dtype=np.float32, na_filter=False, low_memory=False).values
        if train_num_per_sequence != None:
            gt = gt[:train_num_per_sequence]
        #print(gt.shape)
    ...
    def get_sequence_info(self, seq_id, train_num_per_sequence):
        seq_path = self._get_sequence_path(seq_id)
        bbox = self._read_bb_anno(seq_path,seq_id = seq_id,train_num_per_sequence = train_num_per_sequence)

        valid = (bbox[:, 2] > 0) & (bbox[:, 3] > 0)
        visible = torch.ones(bbox.shape[0])
    ...
    def _get_frame_path(self, seq_path, frame_id):
        return os.path.join(self.root,'img','{:05}.jpg'.format(frame_id))
        # return os.path.join(seq_path, '{:05}.jpg'.format(frame_id))    # frames start from 1
    ...
    def get_frames(self, seq_id, frame_ids, train_num_per_sequence, anno=None):
        #print('-------------train_num_per_sequence-------------:',train_num_per_sequence)
        seq_path = self._get_sequence_path(seq_id)
        # obj_meta = self.sequence_meta_info[self.sequence_list[seq_id]]
        ...
        if anno is None:
            anno = self.get_sequence_info(seq_id, train_num_per_sequence = train_num_per_sequence)
