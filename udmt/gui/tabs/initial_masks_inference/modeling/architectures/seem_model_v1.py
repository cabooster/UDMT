...
class GeneralizedSEEM(nn.Module):
    ...
    def forward_seg(self, batched_inputs):
        ...
        # Only add false_positive_mask if it exists (for spatial query tasks)
        if 'false_positive_mask' in outputs:
            extra['false_positive_mask'] = outputs['false_positive_mask']
    ...
    def prepare_next_spaital_mask(self, outputs, batched_inputs, mode='best'):
        ...
        # Remove extra dimension from each gt_mask before processing
        processed_gt_masks = []
        for gt_mask in gt_masks:
            if gt_mask.dim() == 2:
                # [H, W] -> [1, H, W]
                processed_gt_masks.append(gt_mask.unsqueeze(0))

            elif gt_mask.dim() == 3:
                # [1, H, W] or [C, H, W] — assume correct
                processed_gt_masks.append(gt_mask)

            elif gt_mask.dim() == 4:
                # [1, 1, H, W] -> squeeze batch -> [1, H, W]
                if gt_mask.shape[0] == 1 and gt_mask.shape[1] == 1:
                    processed_gt_masks.append(gt_mask.squeeze(0))
                else:
                    print(f"[WARN] Unexpected 4D mask shape: {gt_mask.shape}, skipping.")
            else:
                print(f"[WARN] Skipped invalid mask with shape: {gt_mask.shape}")
            # if gt_mask.dim() == 3 and gt_mask.shape[0] == 1:
            #     # Remove the extra batch dimension added by unsqueeze(0)
            #     processed_gt_masks.append(gt_mask.squeeze(0))
            # elif gt_mask.dim() == 2:
            #     # If it's 2D, add a channel dimension to make it 3D (C, H, W)
            #     processed_gt_masks.append(gt_mask.unsqueeze(0))
            # else:
            #     # For other cases, ensure it's 3D
            #     processed_gt_masks.append(gt_mask.squeeze().unsqueeze(0))
        for i, mask in enumerate(processed_gt_masks):
            if not isinstance(mask, torch.Tensor):
                # print(f"[DEBUG] mask[{i}] is not a tensor:", type(mask))
                pass
            else:
                # print(f"[DEBUG] mask[{i}].shape:", mask.shape)
                pass
        gt_masks = Spatial_ImageList.from_tensors(processed_gt_masks, self.size_divisibility).tensor
        
        # Remove extra dimension if present (should be [bs, ns, h, w] not [bs, ns, 1, h, w])
        if gt_masks.dim() == 5 and gt_masks.shape[2] == 1:
            gt_masks = gt_masks.squeeze(2)  # Remove the extra dimension
