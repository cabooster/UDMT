...
class AttentionDataStruct(nn.Module):
    ...
    def self_attn(self, bs, num_heads):
        ...
        # Ensure memories_spatial is included if it exists in attn_variables and self_attn_dict
        if 'memories_spatial' in self.attn_variables and 'memories_spatial' not in self_attn_name:
            # Check if memories_spatial should be in self attention based on config
            for key, value in self.self_attn_dict.items():
                if 'memories_spatial' in value and key in self_attn_name:
                    self_attn_name.append('memories_spatial')
                    break
        ...
        if 'memory_indices' in self.extra and 'memories_spatial' in self.query_index:
