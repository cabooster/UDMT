...
def point_sample(input, point_coords, **kwargs):
    ...
    if input.dim() == 5 and input.shape[2] == 1:
        input = input.squeeze(2)  # Convert (N, C, 1, H, W) -> (N, C, H, W)
    add_dim = False
    if point_coords.dim() == 3:
        add_dim = True
        point_coords = point_coords.unsqueeze(2)
    # print(f"[DEBUG] input.shape: {input.shape}")         # e.g. (N, C, H, W) or (N, C, D, H, W)
    # print(f"[DEBUG] point_coords.shape: {point_coords.shape}")  # e.g. (N, P, 2) or (N, P, 1, 2)
