...
class SetCriterion(nn.Module):
    ...
    def loss_spatials(self, outputs, targets, indices, num_masks, layer_id, extra):
        ...
        # Normalize all masks to shape [1, H, W]
        normalized_gt_masks = []
        for i, m in enumerate(gt_masks):
            if m.dim() == 4:
                # From [1, 1, H, W] -> [1, H, W]
                if m.size(0) == 1 and m.size(1) == 1:
                    m = m.squeeze(0)  # [1, H, W]
                    gt_masks[i] = m
                else:
                    raise ValueError(f"[ERROR] gt_mask[{i}] unexpected 4D shape: {m.shape}")
            elif m.dim() == 3:
                if m.size(0) != 1:
                    raise ValueError(f"[ERROR] gt_mask[{i}] has unexpected channel dimension: {m.shape}")
                # Already [1, H, W], keep it
            else:
                raise ValueError(f"[ERROR] gt_mask[{i}] invalid shape: {m.shape}")
            
            normalized_gt_masks.append(m)
        #for i, m in enumerate(normalized_gt_masks):
            # print(f"gt_mask[{i}].shape: {m.shape}")
        
        stack_gt_mask = torch.cat(normalized_gt_masks)
        ...
        #     print(f"[DEBUG] gt_masks[{i}].shape: {mask.shape}")
