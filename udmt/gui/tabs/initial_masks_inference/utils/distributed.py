...
def apply_distributed(opt):
    if not MPI_AVAILABLE:
        # Skip distributed setup if MPI is not available
        return
    ...
    if MPI_AVAILABLE:
        ...
        # Use default values for single-process mode
        master_address = opt.get('master_address', '127.0.0.1')
        master_port = opt.get('master_port', '8673')
...
def init_distributed(opt):
    ...
    if not MPI_AVAILABLE or 'OMPI_COMM_WORLD_SIZE' not in os.environ:
        # application was started without MPI or MPI not available
...
def is_main_process():
    if not MPI_AVAILABLE:
        return True  # Always main process if no MPI
